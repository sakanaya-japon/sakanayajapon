...
**2026-09-23 確定**: 上の表は**日本語版**に適用します。**英語版の注文窓口は Telegram #3 のまま**です
（§4「英語版の注文窓口」を参照）。
...
### 英語版の注文窓口（✅ 2026-09-23 確定）

単一情報源化は**遷移先を変えずに**実施したため、日英で着地先が異なる箇所が残っていました。
確認の結果、**英語版は Telegram のままで確定**です。変更は行いません。

| # | 箇所 | 着地先 | 状態 |
|---|---|---|---|
| A | `faq.html` 英語版「How can I place an order?」 | Telegram チャンネル #3（`t.me/fishstoreJapon`） | ✅ 現状維持で確定 |
| B | `aboutus.html` 英語版の CTA ボタン | 同上 #3 | ✅ 現状維持で確定 |

日本語版は LINE Mini App #6、英語版は Telegram #3 という**意図的な使い分け**です。
§2 の統一案「1導線あたり1窓口」は日本語版に適用し、英語版はその例外として扱います。
食い違いではないので、揃えようとしないでください。

将来 #6 に寄せる場合は、`data-info-href` の値を `social.telegramChannel` → `order.lineMiniApp` に
差し替えるだけで済みます（該当2箇所）。
